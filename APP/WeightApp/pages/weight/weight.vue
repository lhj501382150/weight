			...
			<view class="order-box" v-for="(item,index) in orders" :key="item.id">
				...
				<view class="order-box-left">
					...
					<view class="order-title-bott">
						...
						<text class="order-name">{{item.carNumber}}</text>
						<text class="order-name">{{item.orderNo}}</text>
					</view>
					<view class="order-title-bott">
						<text class="order-name1">毛重:{{item.gross}}</text>
						<text class="order-name1">皮重:{{item.tare}}</text>
					</view>
					<view class="order-title-bott">
						<text class="order-name-warn1">净重:{{item.net}}</text>
						<text class="order-name-warn1">结算重量:{{item.balanceWeight}}</text>
					</view>
	...
	export default {
		...
		methods: {
			...
			getWeightList(cb){
				...
				this.$http.post('/weight/findAll',this.pageRequest,(res)=>{
					if(res.code==200){
						var data = res.data;
